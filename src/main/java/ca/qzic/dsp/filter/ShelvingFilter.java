 ...
 * @author Quentin
 */
public class ShelvingFilter implements Command {
...
//        out.println("gain = " + dBgain + ", VO = " + V0 + ", c = " + c);
    ...
    public ShelvingFilter(double sampleRate, double cornerFreq, HS_LS hl_S, double dBgain) {
...
/*
 * function y = lowshelving (x, Wc, G) % y = lowshelving (x, Wc, G) 
 * % Author: M. Holters 
 ...
 * else 
 *  c = (tan(pi*Wc/2)-V0) / (tan(pi*Wc/2)+V0); % cut end; 
 * xh = 0;
 * for n=1:length(x) 
 *  xh_new = x(n) - c*xh; 
 *  ap_y = c * xh_new + xh; 
 *  xh = xh_new; 
 *  y(n) = 0.5 * H0 * (x(n) + ap_y) + x(n);     // change to (x(n) - ap_y) for HS;
 *
 */
